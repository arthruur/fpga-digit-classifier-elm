...
// Predição esperada (obtida do golden model — pred = 2)
parameter [3:0] EXPECTED_PRED = 4'd2;
...
initial begin
    ...
    begin : teste4_scores
        ...
        integer j;
        neg_scores[0] = 16'hF000;  // -1.0
        neg_scores[1] = 16'hE000;  // -2.0
        neg_scores[2] = 16'hFF00;  // -0.25  ← menos negativo
        neg_scores[3] = 16'hC000;  // -4.0
        neg_scores[4] = 16'hF800;  // -0.5
        neg_scores[5] = 16'hF400;  // -0.75
        neg_scores[6] = 16'hF200;  // -0.875
        neg_scores[7] = 16'hD000;  // -3.0
        neg_scores[8] = 16'hFC00;  // -0.25 (igual ao [2])
        neg_scores[9] = 16'hFE00;  // -0.125 ← MÁXIMO (menos negativo)
    ...
    $display("  Escores todos negativos. Máximo em y[9] = -0.125");
    $display("  max_idx = %0d  max_val = %0.5f  (esperado: idx=9, val=-0.125)",
        argmax_max_idx, q412_to_float(argmax_max_val));
    if (argmax_max_idx == 4'd9)
        $display("  [PASS] Argmax signed correto com escores negativos");
    else
        $display("  [FAIL] Erro na comparação signed");
